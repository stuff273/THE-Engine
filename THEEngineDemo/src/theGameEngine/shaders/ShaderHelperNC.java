...
public class ShaderHelperNC extends ShaderHelper{

	public int[] program = new int[]{-1,-1};

	public ShaderHelperNC(){
		String[] lightShaders = {ShadersNC.lightShader0V,ShadersNC.lightShader0F,ShadersNC.lightShader1V,ShadersNC.lightShader1F};
		int shaderHandleV;
		int shaderHandleF;
		...
		for (int i=0; i<program.length; i++){
			...
			GLES20.glBindAttribLocation(program[i],counter++,"a_color");
...
/*		if (drawNumLights>0){
	...
	public void assignHandles(){
		...
		handleColorAttribute = GLES20.glGetAttribLocation(program[0], "a_color");
...
/*		if (drawNumLights>0){
			...
			GLES20.glVertexAttribPointer(colorHandle,4,GLES20.GL_FLOAT,false,model.attributeCount*4,model.indexColor);
